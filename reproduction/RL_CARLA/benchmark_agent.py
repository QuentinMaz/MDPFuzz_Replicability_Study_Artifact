...
def run(args, model_path, port, suite, seed, path, em_guide, replay):
    log_dir = model_path
    total_time = 0.0
    suites  = get_suites(suite)
    print('found {} suites.'.format(len(suites)))
    for suite_name in suites:
        print('found suite', suite_name)
        tick = time.time()
        ...
        with make_suite(suite_name, port=port, crop_sky=args.crop_sky) as env:
            print('running run_benchmark(.)')
            print('is the environment in sampling mode?', env.first_run)
            ...
            run_benchmark(agent_maker, env, seed, path, em_guide=em_guide, replay=replay)
            if args.replay:
