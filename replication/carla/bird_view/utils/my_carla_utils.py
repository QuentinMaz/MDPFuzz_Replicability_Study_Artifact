...
PLAYER_VEHICLE_NAME = "vehicle.lincoln.mkz2017"
    ...
    def tick(self):
        ...
        for ped_controller in self._ped_controllers:
            ...
            while True:
                ...
                if (
                    _loc is not None
                    and _loc.distance(ego_vehicle_location) >= 10.0
...
class CarlaWrapper(object):
    def __init__(
        self,
        town="Town01",
        vehicle_name=PLAYER_VEHICLE_NAME,
        port=2000,
        client=None,
        col_threshold=400,
        seed=None,
        ...
        **kwargs
        ...
        self._player_bp = self._blueprints.filter(vehicle_name)[0]
        self._player_bp.set_attribute("role_name", "hero")
        ...
        self._vehicle_bp = self._blueprints.filter(VEHICLE_NAME)[0]
        self.spawn_point_indices = np.array(
            [
                92, 77, 27, 59, 13, 21,
                67, 99, 4, 26, 36, 11,
                2, 75, 69, 22, 5, 19,
    ...
    def _get_transforms(self, input: np.ndarray = None):
        if input is None:
            spawn_points = self._map.get_spawn_points() # type: List[carla.Transform]
            indices = self.spawn_point_indices
            transform_list = [spawn_points[i] for i in indices[:self.n_vehicles]]
        else:
            assert len(input) == self.n_vehicles * 4
            splits = [arr.copy() for arr in np.array_split(input, self.n_vehicles)]
            transform_list = []
            for arr in splits:
                x, y, z, yaw = arr
    ...
    def _spawn_vehicles(self, transform_list: List[carla.Transform]):
        # The executor class takes care of ensuring that the vehicles are distant enough from each other
        assert len(transform_list) == self.n_vehicles
        for i in range(self.n_vehicles):
            choose_point = transform_list[i]
            vehicle = self._world.try_spawn_actor(self._get_vehicle_blueprint(), choose_point)
            if vehicle is not None:
                vehicle.set_autopilot(True)
                vehicle.start_dtcrowd()
                self._actor_dict["vehicle"].append(vehicle)
            else:
                print('spawn vehicle error')
                with open('spawn_vehicles_error_{}_{}.txt'.format(self.port, time.time()), 'w') as f:
                    print('Actor spawning failed (vehicle {})'.format(i), file=f)
                    print('choose point:', choose_point, file=f)
                    print('pilot:', self._start_pose, file=f)

        # print('Spawn {} vehicles. The world has currently {} ones.'.format(len(self._actor_dict["vehicle"]), len(self._world.get_actors().filter('vehicle.*'))))
    ...
    def spawn_pedestrians(self, n_pedestrians):
        ...
        while peds_spawned < n_pedestrians:
            ...
            batch = [
                SpawnActor(walker_controller_bp, carla.Transform(), walker) for walker in _walkers
            ]
    ...
    def export_positions(self, filename: str, remove_pitch_roll: bool = True):
        '''Exports the positions of all vehicles to a .csv file.'''
        x, y, z, pitch, yaw, roll = [], [], [], [], [], []
        print('found {} players'.format(len(self._actor_dict["player"])))
        for v in self._actor_dict["player"]:
            point = v.get_transform()
            for l, v in zip([x, y, z, pitch, yaw, roll], [point.location.x, point.location.y, point.location.z, point.rotation.pitch, point.rotation.yaw, point.rotation.roll]):
                l.append(v)
        print('found {} vehicles'.format(len(self._actor_dict["vehicle"])))
        for v in self._actor_dict["vehicle"]:
            point = v.get_transform()
            for l, v in zip([x, y, z, pitch, yaw, roll], [point.location.x, point.location.y, point.location.z, point.rotation.pitch, point.rotation.yaw, point.rotation.roll]):
                l.append(v)
    ...
    def _export_transforms(self, filename: str, transform_list: List[carla.Transform], remove_pitch_roll: bool = True):
        x, y, z, pitch, yaw, roll = [], [], [], [], [], []
        for t in transform_list:
            for l, v in zip([x, y, z, pitch, yaw, roll], [t.location.x, t.location.y, t.location.z, t.rotation.pitch, t.rotation.yaw, t.rotation.roll]):
                l.append(v)
        df = pd.DataFrame()
        df['x'] = x
        df['y'] = y
    ...
    def get_situation(self) -> np.ndarray:
        '''Returns the positions of all vehicles as a numpy array of shape (num_vehicles, 6).'''
        positions_list = []
        print('found {} players'.format(len(self._actor_dict["player"])))
        for v in self._actor_dict["player"]:
            point = v.get_transform()
            positions_list.append(
                np.array([point.location.x, point.location.y, point.location.z, point.rotation.pitch, point.rotation.yaw, point.rotation.roll])
            )
        for v in self._actor_dict["vehicle"]:
            point = v.get_transform()
            positions_list.append(
    ...
    def _reset(self, input: np.ndarray = None):
        '''
        Resets the environment.
        Note that resetting the environment does not ensure repeatable executions,
        (autopilot policies are not deterministic for this version).
        If no input is provided, the player is sets to the initial position
        and the vehicles to static ones defined in the class.
        '''
        # if no input is provided, the situation is set to the one defined in self.init(.)
        if input is None:
            assert self._start_index and self._target_index, 'init(.) must be called once (no target otherwise!).'
            self._start_pose = self._map.get_spawn_points()[self._start_index]
        else:
            # weather
            weather_index = int(input[0])
            self.set_weather(PRESET_WEATHERS[weather_index])
            # destination
            self._target_index = int(input[1])
            self._target_pose = self._map.get_spawn_points()[self._target_index]
            # position of the vehicle
            x, y, z, yaw = input[2:6]
            self._start_pose = carla.Transform(
                carla.Location(x=x, y=y, z=z),
                carla.Rotation(pitch=0.0, yaw=yaw, roll=0.0)
            )
            # reduces the input to spawn the vehicles # dirty?
            input = input[6:]
        self.clean_up()
        self.spawn_player()
        self._setup_sensors()
        ...
        if self.seed is not None:
            ...
            # print('Seed reset! ({})'.format(self.seed))

        init_transforms = self._get_transforms(input)
        self._spawn_vehicles(init_transforms)
        ...
        self.peds_tracker = PedestrianTracker(
            weakref.ref(self),
            self.pedestrians,
    ...
    def init(self, start=0, target=1, weather="random"):
        self._start_index = start
        self._start_pose = self._map.get_spawn_points()[start] # libcarla Transform object
        self.clean_up()
        self.spawn_player()
        self._setup_sensors()
        ...
        if self.seed is not None:
            np.random.seed(self.seed)
            # print('Random seed re-seeded! ({})'.format(self.seed))


        self._target_index = target
        self._target_pose = self._map.get_spawn_points()[target]
        # print('Target set to:', self._target_pose)
        self.set_weather(weather)
        # sets the vehicles from static positions
        init_transforms = self._get_transforms(None)
        self._spawn_vehicles(init_transforms)
        ...
        self.traffic_tracker = TrafficTracker(self._player, self._world)
        # ready(.) called is the one of the PointGoalSuite class, which adds a LocalPlanner to track the piloted vehicle
        ready = self.ready()
        # self.export_situation('initial_positions_ready.csv', False)
        return ready


    def spawn_player(self):
        self._player = self._world.try_spawn_actor(self._vehicle_bp, self._start_pose)
        num_attempts = 0
        while (self._player is None) and (num_attempts < 100):
            print('The player could not be spawned. Spawning it elsewhere...')
